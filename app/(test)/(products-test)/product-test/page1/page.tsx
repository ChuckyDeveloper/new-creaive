...
async function ProductsGrid() {
    const data = await fetchProducts("limit=8&sort=-publishedAt,-createdAt");
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
            {data.items.map((it: any) => (
                <Link href={``} key={it._id} className="rounded-xl border border-white/10 bg-white/[0.04] p-0 overflow-hidden">
                    <div className="aspect-[16/10] overflow-hidden border border-white/10">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img
                            // src={it.images?.[0]?.url ?? ""}
                            src={`/api/v1/controllers/getImage/productcontent?name=${encodeURIComponent(it.images?.[0]?.url.replace("/productcontent/", ""))}`}
                            alt={it.title}
                            className="h-full w-full object-cover"
                        />
                    </div>
                    <div className="mt-2 text-sm font-medium px-3">{it.title}</div>
                    <div className="text-xs text-slate-400 px-3 pb-3">{it.subtitle ?? ""}</div>
                </Link>
            ))}
        </div>
    );
    ...
    return (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3">
            {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="animate-pulse rounded-xl border border-white/10 bg-white/[0.03] p-3">
