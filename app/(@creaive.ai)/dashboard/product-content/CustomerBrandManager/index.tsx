...
export default function CustomerBrandManager({ onChanged, baseUrl = "" }: Props) {
    ...
    const buildAssetSrc = useCallback(
        (preview: string) => {
            ...
            if (withoutBase.startsWith("/api/")) {
                return normalizedBaseUrl ? `${normalizedBaseUrl}${withoutBase}` : withoutBase;
            }

            const filename = withoutBase.replace(/^\/?uploads\/brands\/?/, "").replace(/^\/?/, "");
            const prefix = normalizedBaseUrl ? `${normalizedBaseUrl}` : "";
            return `${prefix}/api/v1/controllers/getImage/uploads/brands/?name=${encodeURIComponent(filename)}`;
        },
        [normalizedBaseUrl]
    );
    ...
    if (!brand.open) {
        return (
            <div className="rounded-xl border border-white/10 bg-white/[0.04] p-4 flex items-center justify-between mt-4">
                <div className="text-base font-semibold">Customer Brand</div>
                <button
                    type="button"
        ...
        <div className="space-y-4 rounded-xl border border-white/10 bg-white/[0.04] p-4">
            ...
            <div className="inline-flex rounded-lg border border-white/10 bg-black/30 p-1">
                {(["select", "create"] as const).map((key) => (
                    <button
                        key={key}
                        type="button"
                        onClick={() => dispatch(setBrandTab(key))}
                        className={[
                            "px-3 py-1.5 text-sm rounded-md",
                            brand.tab === key ? "bg-fuchsia-600 text-white" : "text-slate-300 hover:bg-white/10",
                        ].join(" ")}
                    >
                        {key === "select" ? "Select Brand" : "Create New"}
                    </button>
                ))}
            </div>
            ...
            {brand.tab === "select" ? (
                <div className="space-y-4">
                    {/* dropdown */}
                    <div>
                        <label className="text-xs text-slate-400 px-3 pb-3">Select brand</label>
                        <div className="mt-1 flex gap-2">
                            ...
                            >
                                <option value="">{brand.loadingList ? "Loading…" : "— Select —"}</option>
                                {brands.map((b) => (
                    ...
                    {brand.selectedId ? (
                        <BrandForm
                            mode="edit"
                            busy={brand.busy === "update" || brand.busy === "delete" || brand.loadingOne}
                            name={brand.name}
                            setName={(v) => dispatch(setBrandField({ field: "name", value: v }))}
                            slug={brand.slug}
                            setSlug={(v) => {
                            ...
                            description={brand.description}
                            setDescription={(v) => dispatch(setBrandField({ field: "description", value: v }))}
...
function BrandForm(props: {
    ...
    logoPreview: string; logoUrl?: string; setLogoFile: (f: File | null) => void;
    coverPreview: string; coverUrl?: string; setCoverFile: (f: File | null) => void;
    buildAssetSrc: (preview: string) => string;
}) {
    const {
        mode, busy,
        name, setName, slug, setSlug, description, setDescription,
        logoPreview, setLogoFile, logoUrl,
        coverPreview, setCoverFile, coverUrl,
        buildAssetSrc
    } = props;

    const logoSrc = useMemo(() => buildAssetSrc(logoPreview), [buildAssetSrc, logoPreview]);
    const coverSrc = useMemo(() => buildAssetSrc(coverPreview), [buildAssetSrc, coverPreview]);

    return (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1.1fr_1fr]">
            <div className="space-y-4 px-0">
                <h3 className="mb-1 text-sm font-semibold text-white">{mode === "create" ? "New Brand" : "Edit Brand"}</h3>
                ...
                <div className="px-0 py-4">
                    ...
                    <div className="flex flex-col gap-2 py-2 ">
                        ...
                        {logoUrl && !logoPreview.startsWith("blob:") &&
                            <span className="text-xs text-slate-400 px-3 pb-3">current: <em>{logoUrl}</em></span>}
                    </div>
                </div>
                ...
                <div className="px-0 py-4">
                    ...
                    <div className="flex flex-col gap-2 py-2 ">
                        ...
                        {coverUrl && !coverPreview.startsWith("blob:") &&
                            <span className="text-xs text-slate-400 px-3 pb-3">current: <em>{coverUrl}</em></span>}
                    </div>
                </div>
            </div>
        </div>
