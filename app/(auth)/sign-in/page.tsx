...
import {
  ...
  setErrors,
  resetForm,
} from "@/lib/features/forms/signInForm";
import {
  EyeIcon,
  EyeOffIcon,
  GoogleIcon,
...
function SignInClient() {
  ...
  useEffect(() => {
    ...
    if (v !== null)
      dispatch(setField({ field: "rememberMe", value: v === "true" }));
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const result = await dispatch(
      signIn({ identifier: identifier.trim(), password, rememberMe }),
    );
            ...
            <form onSubmit={onSubmit} noValidate className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    dispatch(
                      setField({ field: "identifier", value: e.target.value }),
                    )
                  }
                  onKeyDown={(e) => {
                  ...
                  className={`block w-full rounded-xl border bg-black/30 px-4 py-3 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 ${
                    fieldErrors.id
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`block w-full rounded-xl border bg-black/30 px-4 py-3 pr-12 text-slate-100 placeholder:text-slate-500 focus:outline-none focus:ring-2 ${
                      fieldErrors.pwd
                        ? "border-red-500/50 focus:ring-red-500/40"
                ...
                {capsLock && !fieldErrors.pwd && (
                  <p id="capswarn" className="mt-1 text-xs text-amber-300">
                    Caps Lock is on.
                  </p>
                )}
                {fieldErrors.pwd && (
                  <p id="pwd-error" className="mt-1 text-xs text-red-300">
                    {fieldErrors.pwd}
                  </p>
                )}
              </div>
              ...
              >
                <span
                  ...
                  role={isLoadingUI ? "status" : undefined}
                  aria-live={isLoadingUI ? "polite" : undefined}
                >
                  {isLoadingUI ? (
                    <>
                      <Spinner /> Signing in...
