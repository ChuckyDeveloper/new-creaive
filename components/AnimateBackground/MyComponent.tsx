...
const MyComponent: React.FC = () => {
  ...
  useEffect(() => {
    // Wait one frame so the canvas is fully laid-out in the DOM
    const raf = requestAnimationFrame(() => {
      const canvas = canvasRef.current;
      if (!canvas || !canvas.offsetParent) return;

      try {
        // Dynamic import avoids SSR issues with particlesjs
        import("particlesjs").then((mod) => {
          const Particles = mod.default;
    ...
    return () => {
      ...
      if (
        ...
        typeof particlesRef.current.destroy === "function"
      ) {
  ...
  const text_1 =
    "text-white text-start italic text-[18px] leading-none md:text-[24px] xl:text-[28px] 2xl:text-[40px]";
      ...
      <section className="nav w-full h-full m-auto relative flex items-center justify-center">
        ...
        <div className="w-full h-auto max-w-[1280px] m-auto p-4">
          <div className="ml-0 mx-auto flex flex-col">
            ...
            <a className={text_1}>WHERE CREATIVITY AND AI CONVERGE</a>
          </div>
        </div>
              ...
              >
                <div className="w-full h-full rounded-[6px] bg-gradient-to-r from-primary-600 via-primary-400 to-complementary-500 pl-[2px] pr-[1px] py-[1px]">
                  <div className="h-full w-full items-center justify-center bg-[#121212] rounded-[6px] flex">
                    <div
                      className={`uppercase transition-all duration-300 font-unitea text-white hover:text-[15px] text-[9px] xl:text-[14px] px-2`}
                    >
                      request a demo
                    </div>
                    <MdArrowForward
                      size={20}
                      className="relative text-complementary-500"
                    />
                  </div>
                </div>
              </div>
